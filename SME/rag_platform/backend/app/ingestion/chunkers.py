...
def code_chunk(content: str, max_lines: int = 90, overlap_lines: int = 12) -> list[str]:
    # Adaptive chunking: prefer logical blocks (class/function) before fallback windows.
    ...
    for idx in range(len(starts) - 1):
        block = content[starts[idx]:starts[idx + 1]].strip('\n')
        if block:
            blocks.append(block)
...
def resolve_document_type(path: str) -> str:
    ...
    mapping = {
        '.py': 'code', '.md': 'markdown', '.yaml': 'yaml', '.yml': 'yaml', '.json': 'config',
        '.sql': 'sql', '.txt': 'text', '.pdf': 'pdf', '.docx': 'docx', '.toml': 'config', '.ini': 'config'
    }
