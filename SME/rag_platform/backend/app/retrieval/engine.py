...
def vector_search(db: Session, embedding: list[float], top_k: int = 12):
...
def keyword_search(db: Session, query: str, top_k: int = 12):
...
def reciprocal_rank_fusion(rank_sets: list[list[dict]], rrf_k: int = 60) -> list[dict]:
...
def hybrid_merge(vec_rows: list[dict], key_rows: list[dict], top_k: int = 10, rrf_k: int = 60):
    ranked = reciprocal_rank_fusion([vec_rows, key_rows], rrf_k=rrf_k)
    return ranked[:top_k]
...
def rerank(rows: list[dict], query: str) -> list[dict]:
    ...
    for row in rows:
        ...
        source_boost = 0.02 if meta.get('service_name') else 0.0
        row['score'] = row['score'] + overlap * 0.01 + source_boost
...
def diversify_by_source(rows: list[dict], top_k: int, max_per_file: int = 3) -> list[dict]:
    ...
    for row in rows:
        meta = row.get('metadata', {}) or {}
        key = meta.get('file_path') or row.get('chunk_id') or 'unknown'
...
def compress_context(rows: list[dict], max_chars: int = 7000):
    chunks = []
    used = 0
    for row in rows:
        c = row['content']
