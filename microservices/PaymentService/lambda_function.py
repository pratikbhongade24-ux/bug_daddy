...
def process_refund(payload, context, request_id):
    ...
    # Prevent partial refunds for AXIS bank due to API bug that causes overpayments
    if bank_code == "AXIS" and refund_type == "PARTIAL":
        return error_response(context, request_id, "processRefund", payload, 400,
                              "AXIS bank does not support partial refunds. Please use FULL refund type or select a different bank.")
