...
def parse_statement(payload):
    ...
    # Guard test‑bug flag – only active when explicit env var is set
    if payload.get("simulateBug") == "negative_pages" and os.getenv("ENABLE_BUG_SIMULATION") == "true":
...
def build_transactions(statement, payload):
    ...
    # Safe handling of the amount_cast bug‑injection flag
    if payload.get("simulateBug") == "amount_cast":
        if os.getenv("ENABLE_BUG_SIMULATION") == "true":
            # Preserve original behaviour for controlled testing environments
            raise ValueError("Simulated bug: amount_cast")
        # In production we simply skip the faulty code path
    return transactions
...
def summarize_cashflow(payload, context, request_id):
    ...
    if payload.get("simulateBug") == "missing_bucket" and os.getenv("ENABLE_BUG_SIMULATION") == "true":
        {}["summary"]
    return response(
        context,
        request_id,
        ...
        {
            "summary": {
                "avgMonthlyCredit": credit,
                "avgMonthlyDebit": debit,
                "stability": "GOOD",
            },
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Structured error response – do not re‑raise to avoid 500s
        err_body = {
            "service": SERVICE_NAME,
